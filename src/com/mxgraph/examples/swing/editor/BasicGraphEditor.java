	...
	public BasicGraphEditor(String appTitle, List<String> objectTypes,  mxGraphComponent component)
	{
		...
		while (i<objectTypes.size()) {
			...
			graph.prepareContext(parent, j.toString(), objectTypes.get(i), (int) Math.round(pageFormat.getWidth() * pageScale/11), ((int) Math.round(pageFormat.getHeight()
					* pageScale)/objectTypes.size()/4)+((int) Math.round(pageFormat.getHeight()
					* pageScale)/objectTypes.size()/2)*i,
					(int) Math.round(pageFormat.getWidth() * pageScale *0.8),
					((int) Math.round(pageFormat.getHeight() * pageScale)/objectTypes.size()/2),"swimlane;horizontal=false","objContainer");
			i++;
			j++;
		}
		graph.prepareContext(parent, "97","Activity", (int) Math.round(pageFormat.getWidth() * pageScale/11), ((int) Math.round(pageFormat.getHeight()
				...
				* pageScale)/objectTypes.size()/2)*i,
				(int) Math.round(pageFormat.getWidth() * pageScale *0.8), ((int) Math.round(pageFormat.getHeight()
						* pageScale)/objectTypes.size()/2),"actSwimlane;horizontal=false","actContainer");
		
//		graph.prepareContext(parent, "98","Time", (int) Math.round(pageFormat.getWidth() * pageScale/11), ((int) Math.round(pageFormat.getHeight()
//				* pageScale)/objectTypes.size()/4)+((int) Math.round(pageFormat.getHeight()
//				* pageScale)/objectTypes.size()/2)*(i+1),
//				(int) Math.round(pageFormat.getWidth() * pageScale *0.8), ((int) Math.round(pageFormat.getHeight()
//						* pageScale)/objectTypes.size()/6),"timeSwimlane;horizontal=false","timeContainer");
...
//						* pageScale)/objectTypes.size()/6), "image;image=/com/mxgraph/examples/swing/images/time.png");
//
		...
		jb2.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				while (i<objectTypes.size()) {
					// Print the elements of ArrayList
					graph.prepareContext(parent, j.toString(), objectTypes.get(i), (int) Math.round(pageFormat.getWidth() * pageScale/11), ((int) Math.round(pageFormat.getHeight()
							* pageScale)/objectTypes.size()/2)+((int) Math.round(pageFormat.getHeight()
							* pageScale)/objectTypes.size())*i,
							(int) Math.round(pageFormat.getWidth() * pageScale *0.8),
							((int) Math.round(pageFormat.getHeight() * pageScale)/objectTypes.size()/2),"swimlane;horizontal=false","objContainer");
					i++;
					j++;
				...
				graph.prepareContext(parent, "98","Time", (int) Math.round(pageFormat.getWidth() * pageScale/11), ((int) Math.round(pageFormat.getHeight()
						* pageScale)/objectTypes.size()/2)+((int) Math.round(pageFormat.getHeight()
						* pageScale)/objectTypes.size())*(i+1),
						(int) Math.round(pageFormat.getWidth() * pageScale *0.8), ((int) Math.round(pageFormat.getHeight()
								* pageScale)/objectTypes.size()/3),"timeSwimlane;horizontal=false","timeContainer");
...
//				graph.addCell(ports.get(3),(mxCell) timeLine);
		...
		JSplitPane outer = new JSplitPane(JSplitPane.VERTICAL_SPLIT,graphComponent, libraryPane);
