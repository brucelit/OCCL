...
public class ActOrderViolationChecker {
    ...
    public String firstEvtId;
    public String secondEvtId;

    public ViolatedSet vs = new ViolatedSet();

    public ActOrderViolationChecker() {
    }

    public ActOrderViolationChecker(XLog log,
                                    String refObj,
                                    String firstAct,
                                    String secondAct,
    ...
    public ViolatedSet checkOrderRelation(UIPluginContext context){
        ...
        switch (orderRelation){
            case "coexistence":
                return getCoexistenceRelation(context);
            case "notCoexistence":
                return getNotCoexistenceRelation(context);
            case "direct after":
    ...
    public ViolatedSet getCoexistenceRelation(UIPluginContext context) {
        context.getProgress().setMinimum(0);
        context.getProgress().setMaximum(log.size());
        context.getProgress().setCaption("Constructing hello worlds string");
        context.getProgress().setIndeterminate(false);

        Boolean firstActExistFlag;
        Boolean secondActExistFlag;

        // iterate each case in the log
        for (int i = 0; i < log.size(); i++) {
            // get the current trace
            XTrace tempTrace = log.get(i);
            firstActExistFlag = false;
            secondActExistFlag = false;
            ...
            for (int j = 0; j < tempTrace.size() - 1; j++) {
                ...
                if (tempTrace.get(j).getAttributes().get("concept:name").toString().equals(firstAct)) {
                    firstActExistFlag = true;
                    firstEvtId = tempTrace.get(j).getAttributes().get("event_id").toString();
                }
                if (tempTrace.get(j).getAttributes().get("concept:name").toString().equals(secondAct)) {
                    secondActExistFlag = true;
                    secondEvtId = tempTrace.get(j).getAttributes().get("event_id").toString();
                }
            }
            if ((firstActExistFlag && !secondActExistFlag)) {
                reason = "Activity " + secondAct +" does not coexist with activity "+firstAct;
                vs.appendViolatedRule(firstEvtId, secondAct, refObj, reason);
            }
            else if ((!firstActExistFlag && secondActExistFlag)) {
                reason = "Activity " + firstAct +" does not coexist with activity "+ secondAct;
                vs.appendViolatedRule(firstEvtId, secondAct, refObj, reason);
            }
    ...
    public ViolatedSet getNotCoexistenceRelation(UIPluginContext context) {
        context.getProgress().setMinimum(0);
        context.getProgress().setMaximum(log.size());
        context.getProgress().setCaption("Constructing hello worlds string");
        context.getProgress().setIndeterminate(false);
        ...
        for (int i = 0; i < log.size(); i++) {
            ...
            if ((firstActExistFlag && secondActExistFlag)) {
                reason = "Activity " + secondAct
                        +" (Event Id: "+ secondEvtId+ ") coexist with activity "
                        +firstAct + " (Event Id: "+ secondEvtId+ ").";
                vs.appendViolatedRule(firstEvtId, secondAct, refObj, reason);
            }
